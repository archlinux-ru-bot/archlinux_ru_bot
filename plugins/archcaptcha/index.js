...
async function onNewMessage(bot, msg) {
	...
	if(typeof msg.new_chat_members !== 'undefined') {
		for(let newUser of msg.new_chat_members) {
			...
			try {
				captchaCode = (await exec(VERIFY_COMMAND)).stdout.trim();
			} catch(e) {
				void 0;
			}

			if(captchaCode.length === 0) {
				return bot.sendMessage(msg.chat.id, '<b>Ошибка:</b> у меня нет пакмана для капчи :(', {
					parse_mode: 'HTML',
					reply_to_message_id: msg.message_id
				});
			}

			let captchaMessage = await bot.sendMessage(msg.chat.id,
				`Напиши в ответ на это сообщение вывод команды <code>${VERIFY_COMMAND}</code> и я тебя пропущу. Иначе - кикну через две минуты.
Если ты со смартфона, то, надеюсь, у тебя стоит Arch Linux в Termux, если нет - заходи по возвращению к арчику.
Повторный заход возможен через минуту.`, {
		...
		if(currentCaptcha.length > 0 && typeof msg.text !== 'undefined' && currentCaptcha[currentCaptcha.length - 1].code === msg.text) {
			await delCurrentCaptcha(bot, allCurrentCaptcha);
